...
*** ZSH
    ...
    completion scripts and =.zshenv=) and is fairly brief. I think it provides
    sane defaults, while still being aligned with the 'standard operating behavior'
    of a new =.zshrc=.
    
    I use built-in functions or /small/ shell scripts to provide features that whole
    shell scripts are written for in other ZSH 'configuration frameworks':

    - Changing prompt color when in Insert/Normal mode
    - Displaying current Git, CVS and Mercurial working branch.
    - Autocompleting for any string in the Tmux screen.
    #+CAPTION: zshrc
    #+NAME:   toprc.jpg
    [[./data/zsh.jpg]]
*** [[http://www.vim.org/][Vim]]
    These dotfilez come with a barebones Vim configuration. I used Vim for 10
    years before switching to Emacs. I still have unresolved feelings about it
    and I think that it's still the superior option for quick-n-dirty system
    editing.
*** babel
*** Top
